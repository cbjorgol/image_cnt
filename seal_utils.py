...
class SeaLionData(object):
    def __init__(self, sourcedir, datadir, verbosity):
    ...
    def coords(self, train_id):
        ...
        for cls, color in enumerate(self.cls_colors):
            ...
            has_color = np.sqrt(
                np.sum(np.square(dot_img * (img_diff > 0)[:, :, None] - color_array), axis=-1)) < MAX_COLOR_DIFF
    ...
    def save_sea_lion_chunks(self, coords, folder, chunksize=128):
